...
import { formatViewCount, formatPublishedDate, Video } from '@/lib/youtube';
import VideoPlayer from '@/components/VideoPlayer';
import VideoCard from '@/components/VideoCard';
import { ThumbsUp, ThumbsDown, Share2, Eye, MessageCircle, Heart, Download, Minimize2 } from 'lucide-react';
...
export default function WatchPageClient({ videoId, video, relatedVideos }: WatchPageClientProps) {
  const { setCurrentVideo, setIsMiniPlayerActive } = useVideo();
  const router = useRouter();

  // Set current video when page loads
  useEffect(() => {
    if (video) {
      setCurrentVideo({
        id: videoId,
      ...
      // Disable mini player when on watch page
      setIsMiniPlayerActive(false);
    }
  }, [videoId, video, setCurrentVideo, setIsMiniPlayerActive]);

  const handleMinimize = () => {
    // Enable mini player and navigate to home
        ...
        <div className="premium-card p-4 sm:p-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="glow-button px-4 sm:px-6 py-3 rounded-xl font-bold text-sm text-black flex items-center gap-2 transition-all group"
